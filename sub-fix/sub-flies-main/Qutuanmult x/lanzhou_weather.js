...
$task.fetch({ url: URL, method: "GET" }).then(
  (response) => {
    try {
      ...
      const today = rows[0];
      const headerLine =
        `<div style="font-size:26px;font-weight:700;color:#0F172A;margin-bottom:3px">${today.icon} ${today.tMin}~${today.tMax}° · ${today.text} · 💧${today.precip}%</div>` +
        `<div style="font-size:15px;color:#94A3B8;margin-bottom:8px">更新 ${nowStamp()}</div>`;

      const dayLines = rows.map((r, i) => {
        const bg = i === 0 ? "background:#F0F9FF;" : "";
        return `<div style="${bg}padding:8px 4px;font-size:22px;color:#0F172A">` +
      ...
      const html =
        ...
        headerLine +
        dayLines +
        `<div style="font-size:13px;color:#CBD5E1;margin-top:6px">Open-Meteo · 兰州</div>` +
        `</div>`;
